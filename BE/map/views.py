...
class UpdateRealTimeLocationView(APIView):
    ...
    def patch(self,request,schedule_id):
        is_participant = Participants.objects.filter(schedule_id=schedule_id, participant=request.user).exists()
        if not is_participant:
            return Response({"detail": "해당 약속에 참여하지 않은 유저입니다."}, status=403)
        # 위도/경도 타입 및 범위 검증
        try:
    ...
    def get(self,request):
        def event_stream(): # 이 내부 함수는 스트리밍 응답을 위해 무한 루프를 돌며 데이터를 계속 보냄
            while True:
                ...
                # SSE 형식으로 데이터를 전송
                # "data: ...\n\n" 형식은 SSE 프로토콜의 기본 구조
                # 클라이언트에서는 이 데이터를 onmessage 이벤트로 받게 됨   # onmessage 이벤트 : SSE나 WebSocket 연결을 통해 서버로부터 받은 메시지를 클라이언트가 수신할 때 실행되는 이벤트 처리기
